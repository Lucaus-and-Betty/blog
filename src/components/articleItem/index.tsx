...
const ArticleItem: FC<ArticleItemProps> = ({ articleInfo }) => {
  ...
  return (
    <div className='article-item-container' onMouseEnter={showDes} onMouseLeave={closeDes}>
      <div style={{ opacity: show ? 1 : 0 }} className='article-item-des'>
        {show && <Typewriter
          text={articleInfo.desc}
          typingSpeed={5}
          className='article-item-des-content'
        />}
      </div>
        ...
        <div className='article-item-label'>
          {
            articleInfo.label.map((item, index) => (
              <span key={index} className='article-item-label-item'>{item}</span>
            ))
          }
        </div>
